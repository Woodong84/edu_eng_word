...
// [프로필 선택 화면 (로그인)]
// ---------------------------------------------------
function showProfileScreen(viewer = null) {
    profileScreenViewer = viewer;
...
// 이름을 정확히 입력하면 조회 범위 밖의 프로필로도 이동할 수 있다.
// (예: 하위 프로필로 쓰다가 '마스터'를 검색해서 관리자 프로필로 복귀)
// 부분 검색을 허용하지 않는 이유: 목록이 전부 노출되면 범위 제한이 무의미해지기 때문.
function searchProfileByName() {
    const name = $('profile-search').value.trim();
    if (!name) {
        profileSearchResult = null;
        renderProfileList();
        return;
    }
    const match = Profiles.list().find(p => p.name === name);
    if (!match) return showToast('해당 이름의 프로필이 없어요.\n(정확한 이름을 입력해 주세요)');
    profileSearchResult = [match];
    renderProfileList();
}

// 프로필 화면을 연 주체(viewer)에 따라 보이는 프로필 범위를 제한한다.
// - 최초 접속(viewer 없음): 전체 (로그인 화면 역할)
// - 마스터/멋쟁이아빠: 전체
// - 상위 프로필: 자신 + 자신을 상위로 선택한 하위 프로필
// - 하위 프로필: 자신만
function visibleProfilesFor(viewer) {
    const all = Profiles.list();
    if (!viewer) return all;
    if (ADMIN_PROFILE_NAMES.includes(viewer.name)) return all;
    if (!viewer.parentId) return [viewer, ...Profiles.children(viewer.id)];
    return [viewer];
}
...
function renderProfileList() {
    ...
    container.innerHTML = '';
    const profiles = profileSearchResult || visibleProfilesFor(profileScreenViewer);
    ...
    // 보이는 범위 안에서 상위 프로필 아래에 하위 프로필을 묶어 표시한다.
    // (하위 프로필만 보이는 경우처럼 상위가 목록에 없으면 최상위 카드로 표시)
    const visibleIds = new Set(profiles.map(p => p.id));
    const tops = profiles.filter(p => !p.parentId || !visibleIds.has(p.parentId));
    tops.forEach(p => {
        appendCard(p, false);
        profiles.filter(c => c.parentId === p.id).forEach(c => appendCard(c, true));
    });
}
...
function renderQuizCountSelect() {
    ...
    sel.disabled = false;
    const counts = [];
    for (let n = 5; n < max; n += 5) counts.push(n);
    counts.push(max);
...
function bindStaticEvents() {
    ...
    $('profile-search').addEventListener('keyup', (e) => { if (e.key === 'Enter') searchProfileByName(); });
    $('profile-search').addEventListener('input', (e) => {
        if (!e.target.value.trim() && profileSearchResult) { profileSearchResult = null; renderProfileList(); }
    });
    ...
    // 앱 안에서 프로필 화면을 열면 현재 프로필 권한에 맞게 목록이 제한된다
    const openProfileScreenAsViewer = () => {
        const viewer = Profiles.active();
        Profiles.logout();
        showProfileScreen(viewer);
    };
